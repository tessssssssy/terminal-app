...
class Model
    ...
    def self.add_activity(user, type, distance, duration)
        activity = Activity.new(type, distance, duration)
    ...
    def self.calculate_totals(type, month)
        distance = 0
        duration = 0
        ...
        activities.each do |activity|
            ...
            if activity.type == type && month == Date::MONTHNAMES[month_num]
